...
public class FindMissingScriptsEditor : EditorWindow
{
    ...
    private static void FindInGO(GameObject go, string prefabPath)
    {
        ...
        for (int i = 0; i < components.Length; i++)
        {
            if (components[i] == null)
            {
                ...
                string scriptPath = prefabPath + ".meta";
                string guid = GetGuidFromScriptMeta(scriptPath);

                if (!string.IsNullOrEmpty(guid))
                {
                    // Update the corresponding .cs.meta file
                    string scriptFilePath = AssetDatabase.GetAssetPath(components[i]);
                    string scriptMetaPath = scriptFilePath + ".meta";
                    UpdateScriptMetaWithGuid(scriptMetaPath, guid);
                }
                ...
                Debug.LogWarning("Prefab " + prefabPath + " has an empty script attached:\n" + componentPath, go);
            }
        }

        foreach (Transform child in go.transform)
        {
            FindInGO(child.gameObject, prefabPath);
        }
    }

    private static string GetGuidFromScriptMeta(string scriptMetaPath)
    {
        string[] metaLines = System.IO.File.ReadAllLines(scriptMetaPath);
    ...
    private static void UpdateScriptMetaWithGuid(string scriptMetaPath, string guid)
    {
        string[] metaLines = System.IO.File.ReadAllLines(scriptMetaPath);
        ...
        System.IO.File.WriteAllLines(scriptMetaPath, metaLines);
    }
}
